...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    ycoord = np.linspace(border, img.shape[0] - border, nPointsY)
    #duplicate x coordinates for each y coordinate and vice versa
    xx, yy = np.meshgrid(xcoord, ycoord)
    #stack the pair coordinates into a single array of shape (nPointsX*nPointsY, 2)
    vPoints = np.c_[xx.ravel(), yy.ravel()]
    ...
    return vPoints
    ...
    for i in range(len(vPoints)):
        ...
        for cell_y in range(-2, 2):
            for cell_x in range(-2, 2):
                ...
                #create bins from 0 to 2*pi with nBins+1 elements
                bins = np.linspace(0, 2*np.pi, nBins+1)
                #compute the histogram of angles using the bins
                counts = np.histogram(angles, bins=bins)[0]
                #append the histogram to the descriptor of the current grid point
                desc.append(counts)
                ...
...
def create_codebook(nameDirPos, nameDirNeg, k, numiter):
    ...
    for i in tqdm(range(nImgs)):
        ...
        hog = descriptors_hog(img, grd_pts, cellWidth, cellHeight)
        #append the descriptors to the list of features
        vFeatures.append(hog)
        ...
...
def bow_histogram(vFeatures, vCenters):
    ...
    #for each index in Idx, increment the corresponding bin in the histogram
    histo = np.bincount(Idx, minlength=vCenters.shape[0])
    ...
    return histo
...
def create_bow_histograms(nameDir, vCenters):
    ...
    for i in tqdm(range(nImgs)):
        ...
        grd_pts = grid_points(img, nPointsX, nPointsY, border)
        #compute the descriptors for the grid points
        descriptor_hog = descriptors_hog(img, grd_pts, cellWidth, cellHeight)
        #compute the histogram of the descriptors from the image and add it to the list of histograms
        vBoW.append(bow_histogram(descriptor_hog, vCenters))
        ...
